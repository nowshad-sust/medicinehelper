...
<h1>Medicine Helper<h1>
    ...
    <form class="navbar-form navbar-left">
        <button type="button" href="file_list.php" class="btn btn-success">Browse</button>
...
<div class="list-group">
  ...
  <div class="panel-heading">
    ...
    <?php
        $str=NULL;
        if ($handle = opendir('medicines/')) {
        while (false !== ($entry = readdir($handle))) {
        if ($entry != "." && $entry != "..") {
            $str .= $entry." ";
            //echo $entry."</br>";
            //echo '<a href="$entry">medicine</a>'."\n";
            echo "<a style='font-size:17px;' class='list-group-item' href='medicines/$entry'>".substr($entry, 0,-4)."</a>"."</br>";
            }
        }
        closedir($handle);
        }
